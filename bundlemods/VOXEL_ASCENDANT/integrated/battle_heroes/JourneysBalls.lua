-- Adapted from VASC Gen2CaptureBallPresentation; art and animation unchanged.
return function(C)
...
local BALL_SPRITES = {
  ...
  ULTRA_BALL="master_ball", MASTER_BALL="safari_ball",
  SAFARI_BALL="ultra_ball", PARK_BALL="ultra_ball", FAST_BALL="moon_ball",
  LEVEL_BALL="fast_ball", LURE_BALL="level_ball",
  HEAVY_BALL="lure_ball", LOVE_BALL="heavy_ball",
  FRIEND_BALL="love_ball", MOON_BALL="friend_ball",
}
M.ballSprites=BALL_SPRITES
...
local function loadModule(name)
  if name=="src.ui.gen2.BattleAnimView" and C.view then return C.view end
  ...
  if type(bridge) ~= "table" then
    ...
    local function drawBall(view, runner, itemId, minX, minY, image, path)
      ...
      if type(G.translate) == "function" then
        G.translate(x + 8 + lateral, y + 8 + (landed and (1-(runner.presentationScale or 1))*7 or 0))
      end
      if type(G.rotate) == "function" then G.rotate(angle) end
      if G.scale and runner.presentationScale then G.scale(runner.presentationScale) end
      ...
      -- Gen1 applies an SGB post-pass which Gen2 never needed to bypass.
      -- Preserve the existing true-colour skin through that additional pass.
      if C.markTrueColor then
        local x1,y1,x2,y2=math.huge,math.huge,-math.huge,-math.huge
        for _,p in ipairs({{-10,-10},{10,-10},{-10,10},{10,10}})do
          local px,py=p[1],p[2]
          if G.transformPoint then px,py=G.transformPoint(px,py)end
          x1,y1=math.min(x1,px),math.min(y1,py);x2,y2=math.max(x2,px),math.max(y2,py)
        end
        C.markTrueColor(x1,y1,x2-x1,y2-y1)
      end
