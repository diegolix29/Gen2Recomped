-- Compatibility screens and optional in-game OPTIONS rows for Wilds.
--
-- Standalone Wilds can expose START → OPTIONS →
--   POKE FOLLOW EX  → follower control options
--   WILDS OF KANTO  → wild spawn / sprite / ambient / overlay options
-- VASC4J selects dedicated-hub mode instead: the screens remain registered,
-- but Gold/Silver's native OPTIONS list receives no extra rows.
--
...
function SettingsMenus.new(mod, logic, follower, ambient)
  ...
  self._dedicatedHub = false
  self._registered = false
  return self
end

-- A host such as VASC4J can keep these screens registered for compatibility
-- while routing every setting through its own isolated hub. In that mode this
-- module must not append rows to Gold/Silver's native OPTIONS screen.
function SettingsMenus:setDedicatedHub(enabled)
  self._dedicatedHub = enabled == true
  return self
end
...
function SettingsMenus:register()
  ...
  -- Standalone Wilds exposes two OPTIONS rows. VASC4J's dedicated Voxel
  -- Ascendant hub owns those controls instead, leaving native OPTIONS exact.
  if not self._dedicatedHub and mod.hooks and mod.hooks.wrap then
  ...
  if Config.debug(mod) then
    DebugLog.info(mod, self._dedicatedHub
      and "settings screens registered for dedicated hub (native OPTIONS untouched)"
      or "OPTIONS menus registered: POKE FOLLOW EX + WILDS OF KANTO")
  end
end
