-- Gold/Silver KASC/VASC-style skin for Gen1Recomp's Mod Manager and for simple
...
local function customUIEnabled()
  -- `customUI=false` is an intentional tombstone for pre-0.4 wrappers, not a
  -- live master switch.  Do not let it disable the current ORAS menu layer.
  return true
end
...
local function panel(x, y, w, h, r, alpha, s)
  ...
  G.setColor(0.115, 0.082, 0.025, alpha or 0.94)
  roundRect("fill", x, y, w, h, r)
  G.setColor(0.78, 0.82, 0.86, 0.72)
...
local function beginDraw(ww, wh, opaque)
  ...
  if opaque then
    G.setColor(0.045, 0.038, 0.025, 1)
...
local function drawRows(ww, wh, title, subtitle, rows, cursor, engineScroll, opts)
  ...
  for slot = 1, geo.rows do
    ...
    if row.header then
      ...
      G.setColor(on and 0.82 or 0.72, on and 0.56 or 0.76,
                 on and 0.08 or 0.80, on and 0.68 or 0.10)
      roundRect("fill", geo.x + geo.gap, ry, geo.w - geo.gap * 2,
        geo.rowH, geo.r * 0.55)
      if on then
        G.setColor(1.00, 0.84, 0.32, 0.96)
