-- Olivine static-rock model. Independent native map/art guards.
-- Rounded shape follows the reviewed Pewter model; native Champions art differs.
-- Never apply this placement contract to other rooms or moving entities.
local R={}
...
local nativeRows={
 ...
 [6]={1,2,3,6,7,8},[7]={1,2,7,8},[8]={1,8},
 [9]={1,2,3,6,7,8},[10]={1,2,7,8},[11]={1,2,7,8},
 [12]={1,8},[13]={1,8}}
...
local signature=''
function R.matches(map,x,y)
 if type(x)~='number' or type(y)~='number' or x~=math.floor(x) or y~=math.floor(y) or x<0 or x>=10 or y<0 or y>=16 then return false end
 local d=map and map.def
 if not(d and map.id=='OLIVINE_GYM' and d.generation==2 and d.tileset=='TILESET_CHAMPIONS_ROOM'
  and d.width==5 and d.height==8 and d.environment=='INDOOR' and d.outdoor~=true
...
function R.find(map)
 local out={}
 for y=0,15 do for x=0,9 do if R.matches(map,x,y)then out[#out+1]={x=x,y=y}end end end
 return out
end
...
function R.build(S,map,atlas,budget)
 -- Late bounded replacement: do not remove rocks before the generic region
 -- analysis. Doing so changes its repeat inference and raises stage rails to
 -- 48px. Replace only the completed rock runs, preserving every other region.
 local targets={}
 for y=0,15 do for x=0,9 do if R.matches(map,x,y)then
 ...
 local votes,best,bestN={},map:tileAt(8,28),0
 for k,shape in pairs(S.shapeAt)do if shape.flat and shape.class=='ground'then
  local tile=S.tileAt and S.tileAt[k]
  if tile then votes[tile]=(votes[tile]or 0)+1
   if votes[tile]>bestN then best,bestN=tile,votes[tile]end
  end
 end end
 for _,p in ipairs(targets)do
  S.roundStamps[#S.roundStamps+1]={quads=model,mx=p[1]*16+8,my=0,mz=p[2]*16+8,r=8}
  for dy=0,1 do for dx=0,1 do local k=key(p[1]*2+dx,p[2]*2+dy)
   S.skip[k]=true;S.ground[k]=best;S.shapeAt[k]={class='building',art='building',h=0,flat=false,authored=true}
   if S.runs then S.runs[k]=nil end
  end end
  -- Only the two all-44 blocked cap cells share a false 32px run with the
  -- removed rock below. Retain their art/footprint at its native 16px repeat.
  if S.runs and p[2]==6 and(p[1]==3 or p[1]==6)
   and map:cellCollision(p[1],5)==7 and not map:isWarpTileCell(p[1],5)then
   local cap=true
   for dy=0,1 do for dx=0,1 do
    if map:tileAt(p[1]*2+dx,10+dy)~=68 then cap=false end
   end end
   if cap then for dy=0,1 do for dx=0,1 do
    local k=key(p[1]*2+dx,10+dy);local run=S.runs[k]
    if run and run.h==32 and run.peak==32 and run.north==10 and run.front==13
     and run.extent==4 and run.unit==4 and run.rise==0 and not run.door then
     local clean={};for field,value in pairs(run)do clean[field]=value end
     clean.h,clean.peak,clean.front,clean.extent,clean.unit,clean.fromRepeat=16,16,11,2,2,true
     S.runs[k]=clean
    end
   end end end
  end
