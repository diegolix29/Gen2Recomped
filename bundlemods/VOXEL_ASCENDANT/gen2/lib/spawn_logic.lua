...
function SpawnLogic:_onAggressiveAlert(entity, record)
  ...
  ow.emote = {
    npc = entity,
    frames = frames,
    -- Current Gen-2 engines consume `left` in World:stepBody while `frames`
    -- remains the authored duration used by the VASC alert/flee contract.
    -- Supplying both keeps the public engine emote shape intact and prevents
    -- Crystal from subtracting from a nil countdown.
    left = frames,
    onDone = function()
      if safariFlee then
