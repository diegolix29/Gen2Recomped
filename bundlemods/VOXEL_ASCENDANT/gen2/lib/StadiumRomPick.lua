...
local function usesStadium2()
  if type(StadiumInstall.usesStadium2) == "function" then
    local ok, enabled = pcall(StadiumInstall.usesStadium2)
    if not ok then
      ok, enabled = pcall(StadiumInstall.usesStadium2, StadiumInstall)
    end
...
StadiumRomPick.ID = ((V.mod and V.mod.id) or "VASC4J") .. ":stadiumRom"

local function label()
  return usesStadium2() and "STADIUM 2 ROM" or "STADIUM ROM"
end
  ...
  if not StadiumRomPick.canDialog() then
    if game and game.stack then
      game.stack:push(StadiumScreen.newNote(game, label(),
        usesStadium2() and "PUT POKEMON STADIUM 2 HERE:"
          or "PUT STADIUM US 1.0 HERE:",
