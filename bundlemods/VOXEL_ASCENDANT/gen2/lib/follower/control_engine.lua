...
function ControlEngine:makeTrailer(game, ow, x, y, facing, kind, mon, slot)
  ...
  -- Fresh map entries deliberately seed the train on the player's cell.  On
  -- Gen-2 the NPC layer can sort that entity above Gold/Kris, so the follower
  -- looked like a malformed trainer face until the first step.  Instance-wrap
  -- draw and let syncTrailers reveal it only after it has left the trainer.
  local nativeDraw = npc.draw
  if type(nativeDraw) == "function" then
    npc.draw = function(ent, ...)
      if ent._wildsEntryDrawHidden == true then return end
      return nativeDraw(ent, ...)
    end
  end
  return npc
end
  ...
  if dirty then
    ...
    for i, spec in ipairs(want) do
      ...
      if not okNpc or not npc then
        ...
        if opts.spawnAtPlayer then
          npc._wildsEntryDrawHidden = true
          npc.hiddenBody = true
          npc.visibleSprite = false
        end
  ...
  if committed then
    ...
    if not t1 or t1.moving
      ...
      for _, trailer in ipairs(trailers) do
        trailer._wildsEntryDrawHidden = nil
        trailer.hiddenBody = nil
        trailer.visibleSprite = true
      end
    end
