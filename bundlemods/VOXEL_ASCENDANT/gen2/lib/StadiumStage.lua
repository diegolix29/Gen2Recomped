...
function StadiumStage.arena(map)
  ...
  -- Portable Gen-2 stages occupy the whole widescreen viewport.  The legacy
  -- tele rig was solved for Gold's 160x144 picture and can briefly magnify
  -- the isolated 160x144 sprite captures past every screen edge on the exact
  -- send-out -> menu hand-off, before SMART owns its first safe camera.  Use
  -- the already-reviewed wide authored rig as the deterministic fallback;
  -- SMART still replaces it on the following draw and MAP/Kanto arenas keep
  -- their existing camera contracts.
  arena.cam = "wide"
