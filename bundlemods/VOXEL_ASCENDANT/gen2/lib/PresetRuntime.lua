...
local state = {
  ...
  sprites={}, backdrops={}, exactMusic={}, categoryMusic={}, musicIds={},
}
...
local function buildRuntime(reference, manifest)
  ...
  for _, assignment in ipairs(manifest.assignments) do
    ...
    if assignment.group == "music.mapping" then
      ...
      if assignment.group == "battle.backdrop" then
        validTarget = target:match("^battle/backdrops/") ~= nil and stem == slot
      elseif assignment.group == "pokemon.front" then
      ...
      if assignment.group == "battle.backdrop" then
        local key = assignment.slot:upper()
        if runtime.backdrops[key] then return nil, "sprite-assignment-duplicate" end
        runtime.backdrops[key] = asset.assetPath
      else
...
-- Compatibility for immutable Gen-2 selector generations created before the
-- dedicated battle-background plan lane was introduced. New presets resolve
-- through resolveArenaBackdrop below; legacy packages remain readable without
-- weakening the stricter plan parser.
function PresetRuntime.resolveBackdrop(slot, current)
  if not state.valid then return current end
  local result = state.backdrops[tostring(slot or ""):upper()]
  return result or current
end
