...
--
-- The public screen registrations stay available to either host:
--   1. standalone Wilds may add a top-level TEST SPAWN START row;
--   2. VASC4J sets dedicated-hub mode and opens the same screen from ADVANCED.
--
-- The Mod Manager option schema has no button type
-- (ManagerState.OPTION_TYPES = toggle|choice|number|text), so the browser
-- remains an activate action rather than a fabricated option value.
--
...
-- VASC4J exposes TEST SPAWN from the ADVANCED hub section. Keep all preview
-- screens, but do not create a second top-level START entry in that host.
function PreviewBrowser:setDedicatedHub(enabled)
...
function PreviewBrowser:register()
  ...
  if not self._dedicatedHub then
