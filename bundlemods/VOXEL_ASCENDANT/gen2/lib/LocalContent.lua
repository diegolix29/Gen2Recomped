...
LocalContent.LANES = { "sprites", "music", "backdrops" }
...
local laneEnabled = { sprites=true, music=true, backdrops=true }
local LANE_META = {
  sprites={key="contentLane.sprites", label="INJECT SPRITES"},
  music={key="contentLane.music", label="INJECT MUSIC"},
  backdrops={key="contentLane.backdrops", label="INJECT ARENAS"},
}
...
function LocalContent.restore(game)
  ...
  for _, lane in ipairs(LocalContent.LANES) do
    local key = LANE_META[lane].key
    local value = save and save[key]
    if value == nil and loader then value = loader[key] end
    laneEnabled[lane] = value ~= false
  end
  setLegacyState(game, requested)
  providerBattleSong = nil
...
function LocalContent.resolveSprite(kind, current, ctx)
  ...
  if effective == "CUSTOM" and not laneEnabled.sprites then return current end
...
function LocalContent.resolveMusic(current, ctx)
  ...
  local effective, _, provider = currentState()
  if effective == "CUSTOM" and not laneEnabled.music then return current end
  if effective == "CUSTOM" then
    local preset = PresetRuntime.status()
    if preset.valid then
      local base = current
...
local function copyStatus()
  ...
  local out = {
    ...
    providers=ids,
    lanes={sprites=laneEnabled.sprites, music=laneEnabled.music,
      backdrops=laneEnabled.backdrops},
    available={},
  }
...
function LocalContent.laneEnabled(lane)
  return laneEnabled[lane] ~= false
end

function LocalContent.setLane(lane, enabled, game)
  local meta = LANE_META[lane]
  if not meta then return false, "unknown content lane" end
  laneEnabled[lane] = enabled ~= false
  local save, loader = gameBuckets(game)
  if save then save[meta.key] = laneEnabled[lane] end
  if loader then loader[meta.key] = laneEnabled[lane] end
  providerBattleSong = nil
  write(game)
  return true
end

function LocalContent.laneRow(lane)
  local meta = LANE_META[lane]
  if not meta then return nil end
  return {
    id=((V.mod and V.mod.id) or "VOXEL_ASCENDANT") .. ":" .. meta.key,
    label=meta.label,
    value=function() return laneEnabled[lane] and "ON" or "OFF" end,
    step=function(game)
      return LocalContent.setLane(lane, not laneEnabled[lane], game)
    end,
  }
end

function LocalContent.resolveBackdrop(slot, current)
  local effective = currentState()
  if effective ~= "CUSTOM" or not laneEnabled.backdrops then return current end
  return PresetRuntime.resolveBackdrop(slot, current)
end
  ...
  screens:register("VascContentStatus", {
    new=function(game)
      ...
      local items = {
        ...
        {label="INJECT SPRITES", right=status.lanes.sprites and "ON" or "OFF", action="sprites"},
        {label="INJECT MUSIC", right=status.lanes.music and "ON" or "OFF", action="music"},
        {label="INJECT ARENAS", right=status.lanes.backdrops and "ON" or "OFF", action="backdrops"},
      ...
      local menu = mod.ui.ListMenu.new(game, "PRESET & SCOPE", items, {
        onChoose=function(item, active)
          ...
          elseif item.action == "custom" then
            ...
            if active and type(active.close) == "function" then active:close() end
          elseif LANE_META[item.action] then
            LocalContent.setLane(item.action, not laneEnabled[item.action], game)
            if active and type(active.close) == "function" then active:close() end
...
function LocalContent.public()
  return {
    ...
    resolveBackdrop=LocalContent.resolveBackdrop,
    setLane=LocalContent.setLane, laneEnabled=LocalContent.laneEnabled,
    laneRow=LocalContent.laneRow,
