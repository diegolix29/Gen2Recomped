...
-- Read-only, fail-closed policy for optional dynamic sky behind an imported
-- full-frame Arena bitmap.  Consumers receive a fresh table so a compatibility
-- probe cannot mutate the validated profile held by the registry.
function BattleLayout.skyPolicy(context)
  local backdrop = BattleLayout.backdropFor(context)
  if type(backdrop) ~= "table" then return nil end
  local aperture = type(backdrop.skyAperture) == "table"
    and backdrop.skyAperture or nil
  return {
    voxelSky = backdrop.voxelSky == true,
    skyAperture = aperture and {
...
function BattleLayout.public()
  return {
    ...
    skyPolicy=BattleLayout.skyPolicy,
  }
end
