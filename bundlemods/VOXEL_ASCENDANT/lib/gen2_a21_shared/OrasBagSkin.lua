...
local function classifyMarker(key, value, ignoredOwner)
  ...
  if lower == "__vascgen1bagpockets" then return "vasc_gen1_bag" end
...
local function resolveWideLayer(config, g)
  if type(g) ~= "table" or type(g.newCanvas) ~= "function"
    ...
    return nil, "Ascendant Bag staging canvas is unavailable"
  end
  ...
  if not layer then
    ...
    if not ok or not created then
      return nil, tostring(created or
        "Ascendant Bag canvas allocation failed")
    end
...
local function drawNativeWideFallback(config, g, callback)
  if type(g) ~= "table" or type(g.translate) ~= "function"
    ...
    return false, "Ascendant Bag native fallback transform is unavailable"
  end
...
local CHROME_ACCENTS = {
  gold = {
    primary = { 0.92, 0.67, 0.13, 1 },
    bright = { 1.00, 0.80, 0.28, 1 },
  },
  silver = {
    primary = { 0.68, 0.75, 0.84, 1 },
    bright = { 0.88, 0.93, 0.98, 1 },
  },
  crystal = {
    primary = { 0.12, 0.78, 0.95, 1 },
    bright = { 0.42, 0.91, 1.00, 1 },
  },
}
...
local function normalizeChromeAccent(value)
  if type(value) == "string" then value = CHROME_ACCENTS[value:lower()] end
  ...
  return {
    tonumber(primary[1]), tonumber(primary[2]), tonumber(primary[3]),
    tonumber(primary[4]) or 1,
  }, {
    tonumber(bright[1]), tonumber(bright[2]), tonumber(bright[3]),
    tonumber(bright[4]) or 1,
  }
end

local function withChromeAccent(config, list, callback)
  local value = optionValue(config, "resolveChromeAccent", list, nil)
  local primary, bright = normalizeChromeAccent(value)
...
local function drawWidePresentation(list, config)
  ...
  if type(g) ~= "table" or type(g.setColor) ~= "function"
    ...
    error("Ascendant Bag graphics are unavailable", 0)
  end
  local items = rawget(list, "items") or list.items
  if type(items) ~= "table" then
    error("Ascendant Bag items are unavailable", 0)
  end
  ...
  color(g, C.ink)
  -- Keep implementation/style names out of the player-facing chrome.  The
  -- second title remains the native pocket name, so this short brand label
  -- also leaves a stable text budget for German pocket names and money.
  Font.draw("ASCENDANT", 28, 23)
  ...
  if not drawDpBag(g, list, config, pocketId,
    ...
    error("Ascendant Bag atlas frame is unavailable", 0)
  end
  ...
  list.draw = function(self, ...)
    ...
    if liveConfig.wide then
      ...
      local ok, returned = drawWideAtomically(liveConfig, graphics, function()
        return withChromeAccent(liveConfig, self, function()
    ...
    local returned
    local ok, err = xpcall(function()
      returned = packed(withChromeAccent(liveConfig, self, function()
        return drawPresentation(self, liveConfig)
      end))
    end, traceback)
    if state then restoreGraphics(graphics, state) end
    if ok then return unpackValues(returned, 1, returned.n) end
...
M.normalizeChromeAccent = normalizeChromeAccent
