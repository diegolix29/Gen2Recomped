...
local function normalizeBackdrop(value, profile, options)
  ...
  allowedProperties(value, {
    ...
    horizon=true,vanishingPoint=true,masks=true,voxelSky=true,
    skyAperture=true,
  }, path)
  local out = {}
  if value.asset ~= nil then
  ...
  -- Dynamic VASC sky is deliberately opt-in.  An imported bitmap/profile may
  -- expose transparent sky pixels, but that alone must never enable moving
  -- sun/moon/cloud/weather presentation.  `skyAperture` is a normalized
  -- authoring receipt for camera/event providers; the bitmap remains
  -- responsible for being opaque everywhere outside that reviewed opening.
  if value.voxelSky ~= nil then
    out.voxelSky = expectBoolean(value.voxelSky,
                                 pathJoin(path, "voxelSky"))
  end
  if value.skyAperture ~= nil then
    out.skyAperture = normalizeRect(value.skyAperture,
                                    pathJoin(path, "skyAperture"))
    if out.voxelSky ~= true then
      reject("missing-voxel-sky-opt-in", pathJoin(path, "skyAperture"),
             "skyAperture requires voxelSky=true")
    end
  end
