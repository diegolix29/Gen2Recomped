...
local function languageOf(screen)
  ...
  if mod and type(mod.find) == "function" then
    local okUniversal, universal = pcall(mod.find,
      "translation-german-universal")
    if not okUniversal then
      okUniversal, universal = pcall(mod.find, mod,
        "translation-german-universal")
    end
    local boot = okUniversal and universal and universal.exports
...
local function kascCrystalPath(game, mon, kind)
  if not (mod and type(mod.find) == "function" and game and mon
      and not mon._ascMegaForm and not mon.ascMegaForm) then return nil end
...
local function drawMon(screen, x, y, w, h)
  ...
  if not resolved.trueColor then
    ...
    local colors = PaletteFX.monPal(screen.game.data, screen.mon.species)
...
-- Game2 calls a widescreen owner in physical window coordinates.  Scale the
-- reviewed logical card to the largest aspect-preserving size; integer-only
-- steps left nearly a seventh of a 3420px display unused.
