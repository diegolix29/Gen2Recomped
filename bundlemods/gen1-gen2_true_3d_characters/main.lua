-- Gen1 TRUE 3D Characters Unified (v1.1.6 TEST8 features + v1.1.4 Legacy Compatibility)
-- Unified renderer: ordinary NPCs + Nurse Joy + Professor Oak.
-- NPC SCALE TEST1: generic humans + Nurse Joy at 0.90, Professor Oak at 1.05. Player unchanged.

return function(mod)
  ---------------------------------------------------------------------------
  -- Dedicated Nurse/Oak fallback art. Ordinary NPC sprites remain untouched.
  ---------------------------------------------------------------------------
  ...
  local SPRITE_POOL = {
    ...
    SPRITE_MIDDLE_AGED_MAN = "fat_pool",
  ...
  local host = (battleArt and battleArt.exports and battleArt.exports.lib)
    and battleArt or ds
  if not (host and host.exports and host.exports.lib) then
    mod.log:error("Gen1 TRUE 3D Characters requires Battle Art or Dramatic Shape")
    return
  end

  local V = host.exports.lib
  local characterSurface = battleArt and battleArt.exports
    and battleArt.exports.characterRenderers or nil
  ...
  local motionByObject = setmetatable({}, { __mode = "k" })
  local motionById = {}
  local WALK_CYCLE_DISTANCE = 40

  local function motionState(sprite, actor, suppliedId)
    local key = type(actor) == "table" and actor
      or (type(sprite) == "table" and sprite or nil)
    if key then
      local state = motionByObject[key]
      if not state then state = { travel=0 }; motionByObject[key] = state end
      return state
    end
    local id = tostring(suppliedId or "unknown")
    local state = motionById[id]
    if not state then state = { travel=0 }; motionById[id] = state end
    return state
  end

  local function movementPose(sprite, actor, suppliedId, px, py, facing, phase)
    local state = motionState(sprite, actor, suppliedId)
    local resolved = facing or state.facing or "down"
    local x, y = tonumber(px), tonumber(py)

    if phase == 1 and x and y and state.x and state.y then
      local dx, dy = x - state.x, y - state.y
      local distance = math.sqrt(dx * dx + dy * dy)
      if distance > 0.01 and distance <= 20 then
        state.travel = (state.travel or 0) + distance
        if math.abs(dx) > math.abs(dy) then
          resolved = dx > 0 and "right" or "left"
        else
          resolved = dy > 0 and "down" or "up"
        end
    ...
    if x and y then state.x, state.y = x, y end
    state.facing = resolved
    return resolved, state.travel or 0
  end

  local function genericWalkFrame(travel, count)
    local cycle = ((travel or 0) / WALK_CYCLE_DISTANCE) % 1
  ...
  -- MODIFIED: Explicitly accept the sprite parameter to bridge Battle Art and legacy Dramatic Shape
  local function actorSpriteId(actor, suppliedId, sprite)
    if type(suppliedId) == "string" then return suppliedId end
    if actor and type(actor.spriteId) == "string" then return actor.spriteId end
    local def = actor and actor.def or nil
    if def and type(def.sprite) == "string" then return def.sprite end
    local spriteDef = actor and actor.spriteDef or nil
    if spriteDef and type(spriteDef.id) == "string" then return spriteDef.id end
    local rendererDef = actor and actor.sprite and actor.sprite.def or nil
    if rendererDef and type(rendererDef.id) == "string" then return rendererDef.id end
    
    -- LEGACY FIX: Support directly fetching from the raw sprite parameter when actor is nil
    local rawDef = sprite and (sprite.def or sprite)
    if rawDef and type(rawDef.id) == "string" then return rawDef.id end

    return nil
  end

local function genericKey(sprite, actor, suppliedId)
    local def = sprite and (sprite.def or sprite)
    if not def then return nil end
    
    local poolName = SPRITE_POOL[actorSpriteId(actor, suppliedId, sprite)]
      or markerToPool[def.image]
  ...
  local MODEL_FORWARD_YAW = math.pi
  ...
  local function isNurse(sprite, actor, suppliedId)
    local def = sprite and (sprite.def or sprite)
    return actorSpriteId(actor, suppliedId, sprite) == "SPRITE_NURSE"
      or (def and (def.image == nurseMarker or def.id == "SPRITE_NURSE"))
  end
  ...
  local function isOak(sprite, actor, suppliedId)
    local def = sprite and (sprite.def or sprite)
    return actorSpriteId(actor, suppliedId, sprite) == "SPRITE_OAK"
      or (def and (def.image == oakMarker or def.id == "SPRITE_OAK"))
  end
  ...
  ---------------------------------------------------------------------------
  -- ONE character renderer
  ---------------------------------------------------------------------------
  local function drawReplacement(sprite, px, py, facing, phase, flip, gh,
                                 colors, lift, actor, suppliedId)
    local y = (gh or 0) + (lift or 0)
    local travel
    facing, travel = movementPose(sprite, actor, suppliedId, px, py,
                                  facing, phase)

    if isNurse(sprite, actor, suppliedId) then
      if not ensureNurse() then return false end
      local idx, t = nurseFrame()
      local idle = nurseIdle(t)
      local model = worldModel(px, py, y, facing, idle, 0.90)
      Voxel3D.draw(nurse.meshes[idx], nurse.texture, model, 0,
                   nurseSun(px, py, y, facing, idle))
      return true
    end

    if isOak(sprite, actor, suppliedId) then
      if not ensureOak() then return false end
      local mesh = oak.mesh
      local idle = oakIdle()
      if phase == 1 then
        local frame = genericWalkFrame(travel, 12)
        mesh = oakWalkMesh(frame) or mesh
        idle = { x=0, y=0, z=0, yaw=0 }
      end
      local model = worldModel(px, py, y, facing, idle, 1.05)
      Voxel3D.draw(mesh, oak.texture, model, 0, NO_SELF_SHADOW)
      return true
    end

    local key = genericKey(sprite, actor, suppliedId)
    if key then
      if not ensureGeneric(key) then return false end
      local info = MODEL_DEFS[key]
      local mesh = info.mesh
      local idle = genericIdle(key)
      if phase == 1 and info.walkFrames and info.walkFrames > 1 then
        local frame = genericWalkFrame(travel, info.walkFrames)
  ...
  local function contextActor(ctx)
    if type(ctx) ~= "table" then return nil end
    return ctx.actor or ctx.entity or (ctx.pose and ctx.pose.entity)
  end

  local function contextIsPlayer(ctx, actor)
    if type(ctx) ~= "table" then return false end
    if ctx.isPlayer == true or ctx.role == "player"
        or (ctx.pose and ctx.pose.isPlayer == true) then
      return true
    end
    local game = mod.game
    local overworld = game and game.overworld or nil
    local player = overworld and overworld.player or ctx.player
    return actor ~= nil and player ~= nil and actor == player
  end

  local function contextFacing(ctx, actor)
    if type(ctx) ~= "table" then return nil end
    if type(ctx.facing) == "string" then return ctx.facing end
    local pose = ctx.pose
    if type(pose) == "table" and type(pose.facing) == "string" then
      return pose.facing
    end
    if type(actor) == "table" and type(actor.facing) == "string" then
      return actor.facing
    end
    return nil
  end

  local loggedProviderError = false
  local providerSpec = {
    apiVersion = 1,
    id = "gen1_true_3d_characters.npcs.test8",
    name = "Gen1 TRUE 3D NPCs Test 8",
    priority = 200,
    drawEntity = function(ctx)
        local actor = contextActor(ctx)
        if type(ctx) ~= "table" or contextIsPlayer(ctx, actor) then return false end
        local spriteId = actorSpriteId(actor, ctx.spriteId, ctx.sprite)
        local ok, claimed = pcall(drawReplacement, ctx.sprite, ctx.px, ctx.py,
          contextFacing(ctx, actor), ctx.phase, ctx.flip, ctx.groundHeight, ctx.colors,
          ctx.lift, actor, spriteId)
        if ok then return claimed == true end
        if not loggedProviderError then
          loggedProviderError = true
          mod.log:error("Battle Art NPC provider failed for %s: %s",
                        tostring(spriteId), tostring(claimed))
        end
        return false
    end,
    drawShadow = function(ctx)
        local actor = contextActor(ctx)
        if type(ctx) ~= "table"
            or contextIsPlayer(ctx, actor)
            or not isNurse(ctx.sprite, actor,
              actorSpriteId(actor, ctx.spriteId, ctx.sprite))
            or not ensureNurse() or not nurse.shadowMesh then
          return false
        end
        local services = ctx.host
        local sm = services and services.ShadowMap
        if not (sm and sm.draw and sm.snug) then return false end
        local y = (ctx.groundHeight or 0) + (ctx.lift or 0)
        local model = worldModel(ctx.px, ctx.py, y, contextFacing(ctx, actor),
          { x=0, y=0, z=0, yaw=0 }, 0.90)
        sm.draw(nurse.shadowMesh, nurse.texture, sm.snug(model))
        return true
    end,
  }
  ...
  if type(characterSurface) == "table"
      and tonumber(characterSurface.apiVersion) == 1
      and type(characterSurface.register) == "function" then
    local handle, registerErr = characterSurface.register(providerSpec)
    if handle then
      mod.log:info("Gen1 TRUE 3D NPC Test 8 registered with corrected model-forward yaw")
    else
      mod.log:error("Battle Art character registration failed: %s",
                    tostring(registerErr))
    end
    return
  end

  -- Compatibility with older, shared-global Dramatic Shape builds
  ...
  local function unifiedDrawEntity(sprite, px, py, facing, phase, flip, gh, colors, lift)
    if drawReplacement(sprite, px, py, facing, phase, flip, gh, colors, lift) then
      return true
    end
    return previousDrawEntity(sprite, px, py, facing, phase, flip, gh,
                              colors, lift)
  end
  ...
  mod.log:info("Gen1 TRUE 3D Characters legacy renderer hook successfully installed")
end