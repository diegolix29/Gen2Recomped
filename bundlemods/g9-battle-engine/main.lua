...
local function installHappinessEvolution(mod)
  ...
  -- Gen 2 owns friendship evolution natively (EVOLVE_HAPPINESS, called
  -- fn(entry, mon, ctx) by src/core/gen2/Evolution.lua) -- this Gen 1-shaped
  -- registration would neither be invoked the same way nor translate, and
  -- registering it would also shadow gen2_vocab's prefix fix: an exact
  -- "HAPPINESS" live hit beats the EVOLVE_ prefix rule, so
  -- species_evolutions.lua's HAPPINESS rows would resolve to a method Gold
  -- never fires. On Gen 2 the rows are instead translated to the engine's
  -- own EVOLVE_HAPPINESS by species/gen2_vocab.lua; only the battle
  -- friendship nudge below is installed here.
  if not isGen2 then
...
local function installEvolutionItems(mod, itemIds)
  ...
  ItemEffects.use = function(data, save, itemId, target, battle, ...)
    ...
    for _, evo in ipairs(species and species.evolutions or {}) do
      -- Both vocabularies: Gen 1 registers the method as "ITEM" with the
      -- target under `species`; a Gold boot's evolution rows (normalised by
      -- species/gen2_vocab.lua) read "EVOLVE_ITEM" with the target under
      -- `into`. The row is read LIVE at item-use time, so it carries
      -- whichever shape this boot's species_evolution_patch wrote.
      local method = evo.method
      if (method == "ITEM" or method == "EVOLVE_ITEM") and evo.item == itemId then
        matchedSpecies = evo.species or evo.into
        break
      end
    end
...
return function(mod)
  ...
  -- this mod translates Gen 1 id spellings -- MEDIUM_FAST, LEVEL, ITEM,
  -- THUNDER_STONE, METALCLAW -- into the spellings Gold actually registers
  -- (GROWTH_MEDIUM_FAST, EVOLVE_LEVEL, EVOLVE_ITEM, THUNDERSTONE,
  -- METAL_CLAW). Its registry indexes are built lazily on first use, so
  -- loading it here (ahead of the method/item registrations below and both
  -- consumers) is sufficient. On Gen 1 every method is a no-op -- the id
  -- already matches the live registry exactly and is returned unchanged --
  -- so the callers need no generation branch of their own.
  ...
  boot("species_evolution_patch", function()
    ...
    local patchedEvolutions, skippedSpecies, droppedRows, normalisedRows = 0, 0, 0, 0
      ...
      if mod.content.pokemon:get(id) then
        ...
        for _, evo in ipairs(evoList) do
          if mod.content.pokemon:get(evo.species) then
            ...
            -- Translate the row's method/item into the live registry's
            -- vocabulary (Gen 1 no-op; Gold turns LEVEL into EVOLVE_LEVEL
            -- and THUNDER_STONE into THUNDERSTONE). Done on the finished row
            -- so the gen2 shape (into) and the gen1 shape (species) take the
            -- same path, and so a row already spelled correctly is handed
            -- back with its identity intact.
            if vocab then
              local fixed, didChange = vocab.evolutionRow(row)
              row = fixed
              if didChange then normalisedRows = normalisedRows + 1 end
            end
            resolvable[#resolvable + 1] = row
          else
    ...
    mod.log:info(string.format(
      "g9-battle-engine: patched evolutions onto %d species (%d skipped unregistered, %d rows dropped for an unregistered target, %d rows re-spelled for this generation)",
      patchedEvolutions, skippedSpecies, droppedRows, normalisedRows))
