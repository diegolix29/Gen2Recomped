...
-- Forest tilesets that should have canopy (forest ceiling) across all generations
DayNight.FOREST_TILESETS = {
  FOREST = true,
  FOREST_GATE = true,
  -- Gen 2 specific forest tilesets
  TilesetForest = true,
  TilesetIlexForest = true,
  -- Add more as needed for other generations
}

-- Rooftop patterns: maps that are rooftops and should have no ceiling, just sky
DayNight.ROOFTOP_PATTERNS = {
  "ROOF",
  "ROOFTOP",
  "_ROOF",
  "DECK",
...
function DayNight.isCanopy(map)
  ...
  -- Check explicit map IDs first
  if map.id and DayNight.CANOPY[map.id] then
    return true
  end
  
  -- Check by tileset for forest areas
  local tilesetId = (map.def and map.def.tileset) or (map.tileset and map.tileset.id)
  if tilesetId and DayNight.FOREST_TILESETS[tilesetId] then
    return true
  end
  
  -- Check by map name patterns for forests
  local mapId = map.id and tostring(map.id):upper() or ""
  if mapId:find("FOREST") or mapId:find("WOODS") or mapId:find("GROVE") then
    return true
  end
...
function DayNight.isRooftop(map)
  if not map or not map.id then return false end
  
  local mapId = tostring(map.id):upper()
  
  -- Check against rooftop patterns
  for _, pattern in ipairs(DayNight.ROOFTOP_PATTERNS) do
    if mapId:find(pattern) then
      return true
    end
  end
...
-- Cave tilesets that should have cave ceilings across all generations
DayNight.CAVE_TILESETS = {
  CAVERN = true,
  UNDERGROUND = true,
  CEMETERY = true, -- Pokemon Tower
  -- Gen 2 specific cave tilesets
  TilesetCave = true,
  TilesetDarkCave = true,
  TilesetUnionCave = true,
  TilesetMtMoon = true,
  TilesetRockTunnel = true,
  TilesetSeafoam = true,
  TilesetVictoryRoad = true,
  -- Add more as needed
}

-- Cave name patterns for maps that don't use standard tilesets
DayNight.CAVE_PATTERNS = {
  "CAVE",
  "MT_",
  "MOUNT",
  "UNDERGROUND",
...
function DayNight.isCave(map)
  if not map then return false end
  
  -- Check by tileset first
  local tilesetId = (map.def and map.def.tileset) or (map.tileset and map.tileset.id)
  if tilesetId and DayNight.CAVE_TILESETS[tilesetId] then
    return true
  end
  
  -- Check by map name patterns
  local mapId = map.id and tostring(map.id):upper() or ""
  for _, pattern in ipairs(DayNight.CAVE_PATTERNS) do
    if mapId:find(pattern) then
      return true
    end
  end
  
  -- Check Gen3 specific rock_plateau flag if available
  local okG3, Gen3 = pcall(require, "Gen3")
  if okG3 and Gen3 and Gen3.mapIsGen3 and Gen3.mapIsGen3(map) then
    local okG3c, g3c = pcall(Gen3.forMap, map)
    if okG3c and g3c and g3c.profile and g3c.profile.rock_plateau then
      return true
    end
  end
