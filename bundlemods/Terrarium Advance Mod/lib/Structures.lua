...
local function pixels(tileset)
  -- A GEN 3 PAIR HAS NO SHEET ON DISK. Its art is two baked metatile
  -- sheets that live only in the renderer, so `tileset.image` is nil --
  -- and every pass below that reads these pixels (the background flood,
  -- the object carver, the billboard slabs, the hull carving that makes a
  -- tree a tree, the void test, grass, flowers, relief) was silently
  -- skipping itself (all gated on `if data then`), and every tree and
  -- every house fell through to a plain measured box.
  --
  -- Gen3.shapeDataForTileset hands back the same bake in the same 8px
  -- tile-sheet layout this file already reads (`(t % perRow) * 8`), with
  -- ground colours cut to transparent -- the SHAPE surface, not the
  -- texture -- so the alpha tests below (`a == 0` for background) find
  -- real outlines instead of one opaque rectangle.
