...
local function bodySignature(map)
  ...
  -- Gen 3's sheet is a different shape from Gen 1/2's (see ChunkMesher's own
  -- atlas gate) -- without this, a Gen 3 map's cache signature was computed
  -- off the same Gen 1 fallback numbers regardless of the real atlas, so a
  -- stale cache entry baked before an atlas-layout change never invalidated.
  if Gen3 and Gen3.isGen3(ts) then
    local info = Gen3.describe(ts)
    perRow, imgW, imgH = info.perRow, info.width, info.height
  end
  h = hashAdd(h, perRow)
  h = hashAdd(h, imgW)
  h = hashAdd(h, imgH)
