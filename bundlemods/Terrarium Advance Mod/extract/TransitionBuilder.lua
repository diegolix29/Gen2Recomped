...
local function write(mod,path,data,generated,preserve)
  if preserve and Persist then return Persist.write(mod,"transition",path,data,generated,true) end
  local ok,err=mod.cache:write(path,data);assert(ok,err or ("cache write failed: "..path));generated[#generated+1]=path
end
function T.run(mod,disc,progress,generated,opts)
  local preserve=opts and opts.preserveExisting==true
  progress("TRANSITION MASK A",0,2);write(mod,"assets/transition/wipe_ball00.rgba",mask(1),generated,preserve)
  progress("TRANSITION MASK B",1,2);write(mod,"assets/transition/wipe_ball01.rgba",mask(2),generated,preserve)
  progress("TRANSITION READY",2,2)
  return true
end
T._test={write=write,mask=mask}
return T
