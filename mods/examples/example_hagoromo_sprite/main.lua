...
return function(mod)
  ...
  -- User's sprite is 32x191 pixels with 6 frames vertically (1x6 layout)
  -- Each frame is approximately 32x32 pixels (191/6 ≈ 31.83, so likely 32x32 with some padding)
  mod.log:info("Configuring for 32x191 sprite with 6 vertical frames...")
  
  local ok, err = pcall(function()
    mod.content.sprites:register("SPRITE_HAGOROMO", {
      image = spritePath,
      frames = 6,              -- 6 frames total
      frameWidth = 32,         -- 32 pixels wide per frame
      frameHeight = 32,        -- 32 pixels tall per frame (approximate)
  ...
  mod.log:info("Using: 32x191 sprite, 6 frames vertical, 32x32 per frame")
end